...
const MultiStepForm = () => {
  ...
  const handleHospitalSubmit = async (data) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/hospitals`, data, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
  ...
  const handleSummarySubmit = async (data) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/hospitalvisitentries`, hospitalVisitEntry);
